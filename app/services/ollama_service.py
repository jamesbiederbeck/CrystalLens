...
class OllamaService:
    ...
    def _build_analysis_prompt(self, posts: List[Dict], employee_info: Dict) -> str:
        ...
        prompt = f"""
You are an AI privacy analyst helping individuals audit their own social media presence for personal security and privacy risks. 
...
Provide a comprehensive privacy and security assessment based strictly on the social media posts above. Your goal is to help this person identify privacy risks and protect themselves from potential threats.
For every finding, include concise reasoning and cite specific posts using the format [Post <n> - <date>] with a short quoted snippet where applicable.
Focus on actionable insights that help the person improve their privacy and security posture.

1. PRIVACY EXPOSURE SCORE (0-100 scale):
   - 0-29: Low exposure (good privacy practices)
   - 30-59: Medium exposure (some privacy concerns)
   - 60-79: High exposure (significant privacy risks)
   - 80-100: Critical exposure (urgent action needed)
   Provide a brief rationale with citations.

2. PRIVACY RISK ASSESSMENT:
   Analyze what information has been publicly shared that could be used to identify, locate, or harm this person. Include reasoning and citations to specific posts.

3. COMPROMISING CONTENT ANALYSIS:
   Identify any content that could be used as kompromat, taken out of context, or used to damage reputation. Include reasoning and citations.

4. PRIVACY CONCERNS (Red Flags):
   List specific privacy and security issues (each item should include a short reason and citation in-line):
   - Leaked PII (phone numbers, addresses, emails, financial info, etc.)
   - Location tracking patterns (check-ins, location tags, routines)
   - Oversharing of personal information (family details, work schedules, travel plans)
   - Compromising photos or statements
   - Information useful for social engineering attacks
   - Content that could be weaponized against the person

5. POSITIVE PRIVACY PRACTICES:
   List good privacy behaviors observed (each item should include a short reason and citation in-line):
   - Good operational security practices
   - Privacy-conscious posting habits
   - Appropriate boundaries maintained

6. CONFIDENCE LEVEL (0-100):
   How confident are you in this assessment based on the available data?

7. SPECIFIC PRIVACY ASSESSMENTS:
...
{{
    ...
    "character_assessment": "<detailed privacy risk analysis with reasoning and citations>",
    "behavioral_insights": "<privacy behavior patterns and concerns with reasoning and citations>",
    "red_flags": ["<privacy concern> (reason: ..., citation: [Post n - date] 'quote')", "..."],
    "positive_indicators": ["<good privacy practice> (reason: ..., citation: [Post n - date] 'quote')", "..."],
    "confidence_score": <number 0-100>,
    "summary": "<brief summary of key privacy findings>",
    "assessments": {{
        "leaked_pii": "<summary with reasoning and citation(s) if found, else 'none detected'>",
        "compromising_content": "<summary with reasoning and citation(s) if found, else 'none detected'>",
        "reputation_risks": "<summary with reasoning and citation(s) if found, else 'none detected'>",
        "location_tracking_patterns": "<summary with reasoning and citation(s) if found, else 'none detected'>",
        "oversharing_patterns": "<summary with reasoning and citation(s) if found, else 'none detected'>"
    }}
}}

Be objective, professional, and focus on helping the person improve their privacy and security. Provide actionable insights they can use to protect themselves.
"""
...
"""
    ...
    def _build_evidence_prompt(self, posts: List[Dict], employee_info: Dict) -> str:
        ...
        for i, post in enumerate(posts[:80], 1):
            ...
            posts_text += f"Content: {text}\n"
        prompt = (
            "You are extracting structured EVIDENCE from social media posts for a privacy and security self-assessment.\n"
            ...
            "      \"topics\": [\"personal_info\", \"location\", \"family\", \"work\", \"sensitive\", ...],\n"
            "      \"privacy_flags\": [\"pii_leaked\", \"location_shared\", \"oversharing\", \"compromising_content\", \"reputation_risk\"],\n"
            "      \"positive_signals\": [\"good_privacy_practice\", \"appropriate_boundaries\", \"security_conscious\"]\n"
            "    }\n"
            "  ]\n"
            "}\n"
        )
    ...
    def _build_analysis_prompt_from_evidence(self, evidence: Dict[str, Any], employee_info: Dict) -> str:
        ...
        for ev in (evidence.get('posts') or [])[:100]:
            try:
                ...
                pflags = ', '.join(ev.get('privacy_flags') or [])
                psigs = ', '.join(ev.get('positive_signals') or [])
                lines.append(f"[Post {idx} - {date}] '{snip}' | sentiment={sent} | topics={topics} | privacy_flags={pflags} | positive={psigs}")
            except Exception:
                continue

        evidence_block = "\n".join(lines)
        prompt = f"""
You are a privacy analyst. Use ONLY the EVIDENCE below (do not invent). Provide detailed, cited reasoning to help the person protect their privacy.
...
- Focus on helping this person identify privacy risks and take action.
- If unknown, say 'none detected'.

Return ONLY JSON:
{{
  "risk_score": <number 0-100>,
  "character_assessment": "<detailed privacy risk analysis with reasoning and citations>",
  "behavioral_insights": "<compromising content patterns with reasoning and citations>",
  "red_flags": ["<privacy concern> (reason, citation)", "..."],
  "positive_indicators": ["<good privacy practice> (reason, citation)", "..."],
  "confidence_score": <number 0-100>,
  "summary": "<brief privacy findings summary>",
...
"""
    ...
    def _coerce_to_json(self, previous_text: str) -> str:
        ...
        schema = {
            ...
            "assessments": {
                "leaked_pii": "string",
                "compromising_content": "string",
                "reputation_risks": "string",
                "location_tracking_patterns": "string",
                "oversharing_patterns": "string"
            }
        }
        instruction = (
